...
| **THROTTLE_BRAKING_START**          | Distance where the car begins applying throttle or braking during any event                                       | 1) Detect the start of the event where `THROTTLE` or `BRAKE` is greater than a threshold and record the `LAP_DISTANCE` of the first instance.                              |
| **THROTTLE_BRAKING_PEAK**           | Distance where the car reaches peak throttle or braking during any event                                          | 1) Record the `LAP_DISTANCE` of the frame where maximum throttle or braking value is reached during the event.                                                             |
| **THROTTLE_BRAKING_END**            | Distance where the car finishes applying throttle or braking during any event                                     | 1) Detect when `THROTTLE` or `BRAKE` falls below the threshold and record the `LAP_DISTANCE` of the end of the event.                                                      |
