...
| **BRAKE_EVENT_START**               | Distance where the car begins braking during any brake event                                                      | 1) Detect braking event where `BRAKE` > 0.1 and record the `LAP_DISTANCE` of the first braking frame.                                                                      |
| **BRAKE_EVENT_PEAK**                | Distance where the car reaches peak braking (maximum brake value) during a braking event                          | 1) Record the `LAP_DISTANCE` of the frame where maximum brake value is reached during a braking event.                                                                     |
| **BRAKE_EVENT_END**                 | Distance where the car finishes braking during any brake event                                                    | 1) Detect when `BRAKE` ≤ 0.1 and record the `LAP_DISTANCE` where braking ends.                                                                                             |
| **THROTTLE_EVENT_START**            | Distance where the car begins applying throttle during any throttle event                                         | 1) Detect throttle event where `THROTTLE` > 0.1 and record the `LAP_DISTANCE` of the first throttle frame.                                                                 |
| **THROTTLE_EVENT_PEAK**             | Distance where the car reaches peak throttle (maximum throttle value) during a throttle event                     | 1) Record the `LAP_DISTANCE` of the frame where maximum throttle value is reached during a throttle event.                                                                 |
| **THROTTLE_EVENT_END**              | Distance where the car finishes applying throttle during any throttle event                                       | 1) Detect when `THROTTLE` ≤ 0.1 and record the `LAP_DISTANCE` where throttle ends.                                                                                         |
